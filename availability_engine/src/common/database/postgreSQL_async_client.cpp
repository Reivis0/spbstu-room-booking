...
std::map<std::string, std::string> PostgreSQLAsyncClient::Connector::read_config() {
    ...
    if (!f.is_open()) {
        LOG_ERROR("Failed to open config file configs/pg_config.ini");
        return cfg;
    }
...
std::string PostgreSQLAsyncClient::Connector::get_connecting_str() {
    std::ostringstream s;
    s << "host=" << host << " port=" << port << " dbname=" << db_name << " user=" << username << " password=" << password;
    return s.str();
}
        ...
        for (auto &q : m_queue) {
            ...
            switch (q.kind) {
                case PendingQuery::K_BookingsByRoomDate:
                    if (q.bookings_cb) FinishBookingsByRoomDate_(this, q, std::move(empty), false, "client shutting down");
                    break;
                case PendingQuery::K_ConflictsByInterval:
                    if (q.conflicts_cb) FinishConflicts_(this, q, std::move(empty), false, "client shutting down");
                    break;
                case PendingQuery::K_Generic:
                    if (q.generic_cb) FinishGeneric_(this, q, std::move(empty), false, "client shutting down");
                    break;
            }
        }
        m_queue.clear();
...
void PostgreSQLAsyncClient::executeQueryWithTable(const std::string& sql_template, const std::string& table,
    ...
    if (!is_table_allowed(table)) {
        if (cb) cb->onResult(Result(), false, "table not allowed");
        return;
    }
    ...
    if (pos != std::string::npos) {
        sql.replace(pos, placeholder.size(), table);
    }
...
bool PostgreSQLAsyncClient::is_table_allowed(const std::string& table) {
    ...
    for (const auto& t : allowed) if (t == table) return true;
    return false;
}

void PostgreSQLAsyncClient::getBookingsByRoomAndDate(const char* room_code, const char* dayISO, std::unique_ptr<IBookingsByRoomDateCb> cb) {
    static const char* SQL =
        ...
        "AND b.starts_at < ($2::date + INTERVAL '1 day') AND b.ends_at > ($2::date) "
...
void PostgreSQLAsyncClient::begin_async_connect() {
    ...
    if (PQsetnonblocking(m_connect.connection, 1) != 0) {
        m_connect.error_msg = std::string("Failed to set non-blocking: ") + PQerrorMessage(m_connect.connection);
        m_connect.status = ConnectStatus::ERROR;
...
void PostgreSQLAsyncClient::run_loop() {
    while (m_running) {
        ...
        if (m_connect.status == ConnectStatus::CONNECTED) {
            if (pfd.revents & POLLIN) on_readable();
            if (pfd.revents & POLLOUT && !m_inflight) try_send_next(); 
        }
    }
}

void PostgreSQLAsyncClient::on_readable() {
    if (PQstatus(m_connect.connection) != CONNECTION_OK) {
        LOG_ERROR("Socket error/hup detected. Reconnecting...");
        disconnect();
        return;
    }
...
void PostgreSQLAsyncClient::try_send_next() {
                          ...
                          vals.data(), lens.data(), fmts.data(), 0) != 1) 
    {
        ...
        Result empty;
        if (bad.kind == PendingQuery::K_Generic) FinishGeneric_(this, bad, std::move(empty), false, err.c_str());
        else if (bad.kind == PendingQuery::K_BookingsByRoomDate) FinishBookingsByRoomDate_(this, bad, std::move(empty), false, err.c_str());
        else if (bad.kind == PendingQuery::K_ConflictsByInterval) FinishConflicts_(this, bad, std::move(empty), false, err.c_str());
        return;
    }
...
void PostgreSQLAsyncClient::FinishBookingsByRoomDate_(PostgreSQLAsyncClient* self, PendingQuery& q, Result&& r, bool ok, const char* err) {
    auto* cb = q.bookings_cb.get();
    if (!cb) return;
    std::vector<Booking> bookings;
    if (ok) mapResultToBookings(r, bookings);
    cb->onBookings(bookings.data(), bookings.size(), ok, err);
}

void PostgreSQLAsyncClient::FinishConflicts_(PostgreSQLAsyncClient* self, PendingQuery& q, Result&& r, bool ok, const char* err) {
    auto* cb = q.conflicts_cb.get();
    if (!cb) return;
    std::vector<Booking> bookings;
    if (ok) mapResultToBookings(r, bookings);
